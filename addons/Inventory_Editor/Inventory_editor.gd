...
func _enter_tree() -> void:
    ...
    %HighLightTextureButton.pressed.connect(on_click_highlight_texture)
...
func setup_select_dialog():
    ...
    select_dialog.current_dir = "res://Sprite/Inventory"
    select_dialog.add_filter("*.atlastex", "Atlas Texture Files")
...
func timer_over_to_save():
    ...
    current_inventory["highlight_texture"] = %HighLightTexture.texture
...
func set_current(new_inventory: Inventory):
    ...
    print(str(current_inventory["id"]))
    %ID.value = current_inventory["id"]
    %Texture.texture = current_inventory["texture"]
    %HighLightTexture.texture = current_inventory["highlight_texture"]
