...
enum OneShotState {
    ...
    Axe, ## 斧子
    FishingCasting, ## 钓鱼抛竿
    FishingCaptureNoFish, ## 收杆没有鱼
    FishingCaptureFish, ## 收杆有鱼
    FishingRoll, ## 钓鱼收杆
    Petting ## 爱抚
}
...
#func pick_pickable():
    ...
    #var result = add_inventory(pick.inventory, pick.count)
    #if result:
        #PromptManager.message(lift_sprite, pick.inventory.texture, "%s" % [pick.inventory.name])
        #pick.queue_free()
#
#func add_inventory(inventory: Inventory, count: int):
...
#
        ...
        #current = find_can_use_item()
            ...
            #if count > 1:
                #add_inventory(inventory, count - 1)
    #else:
...
#func to_buy_inventory(inventory: Inventory, count: int):
    #var price = inventory.price * count
        ...
        #var current = add_inventory(inventory, count)
        #if current:
